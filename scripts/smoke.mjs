 ...
 *
 * WHY THIS EXISTS SEPARATELY. The 155 tests in tests/ cover logic. Four things
...
const guessed = orderUrl.replace(/[0-9a-f-]{36}$/, "00000000-0000-0000-0000-000000000000");
...
if (isOwner) {
  ...
  await page.goto(`${BASE}/admin/praxi`, { waitUntil: "networkidle" });
  check(
...
for (let visit = 0; visit < 3; visit += 1) {
  const response = await cspPage.goto(BASE, { waitUntil: "networkidle" });
...
await cspPage.goto(`${BASE}/signup`, { waitUntil: "networkidle" });
...
await cspPage.goto(BASE, { waitUntil: "networkidle" });
...
await first.goto(`${BASE}/account#security`, { waitUntil: "networkidle" });
const everywhere = first.getByRole("button", { name: "Sign out everywhere" });
check("the account page offers sign out everywhere", (await everywhere.count()) > 0);
...
await second.goto(`${BASE}/account`, { waitUntil: "networkidle" });
